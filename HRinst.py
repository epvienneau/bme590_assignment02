def HRinst(dataset, secperunit=60, peak_threshold=98, filt=True):
    ...
    thresholded = stats.threshold(voltage, np.percentile(voltage,peak_threshold))
...
def data_filter(voltage, num_coef=8, hfreq=0.125, lfreq=0.0125):
    '''
    Filter the data using a band pass filter to smooth corners out and remove baseline drift
